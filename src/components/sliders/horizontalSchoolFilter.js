...
export default function SliderHorizontalSchoolFilter(props) {
    ...
    const addPostFavorite = (idPost, user_name)=>{
      ...
      const getSchoolWithFilter=()=>{      
        
        // return console.log(`${process.env.WP_URL_REST}/wp/v2/colegio${props.type}`);
          fetch(`${process.env.WP_URL_REST}/wp/v2/colegio?${props.type}&_embed`,{
            method:props.method,
            headers: {
      ...
      const getSchoolsFavorites = ()=>{
            ...
            .then(
              (response) => {
                ...
                // console.log(data);
                // console.log(data.map((element)=> {return [element.id_post]}).reduce((acc, element)=>{ return parseInt(element) },[]));
              } 
            )
            .catch((error)=>console.log(error));
        }
